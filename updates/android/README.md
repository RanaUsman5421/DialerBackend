...
Current release: `lionexphone-1.2.3.apk` (`com.example.lionexphone`, versionCode 5, versionName 1.2.3).
...
`updater.json` points to the GitHub Release asset at tag `android-v1.2.3`, **not** a GitHub `blob/main` page.
...
An existing debug-key-signed install cannot be updated in place with this release-key-signed APK.
Migrate such devices by backing up any needed local data, uninstalling the debug build, and installing the release APK.
